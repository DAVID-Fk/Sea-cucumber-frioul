...
#svg(file="Fig2.svg", width=7.5, height=12)

par(las=1, mar=c(4,6,2,2), bty="n", mfrow=c(2,1), oma=c(0,0,2,0), cex.lab=1.3)

### Part a
boxplot(sedconctot~substring(names(sedconctot), 8, 10), xlab="", ylab=expression(paste("Fatty acids concentration (mg g d", w^-1, ")")), ylim=c(0,2))
text(1:4, tapply(sedconctot, substring(names(sedconctot), 8, 10), max)+0.2, labels=c("c", "b", "bc", "a"), font=1, cex=1.5)
title(main=expression(bold("Sediment")), line=1, cex.main=2)
...
text(acp$co*5.4, labels=colnames(sedPr), col="grey40")

a=ordihull(acp$li, groups=substring(rownames(acp$li), 8, 10), lty=4, label=F, display=c("sites"))
...
### Part a
  ...
  plot(acp$li[,c(1,2)], acp$co[,c(1,2)], type="n", ann=F, bty="n", axes=F, xlim=c(-7,7), ylim=c(-5,5))
  ...
  arrows(x0=0, y0=0, x1=acp$co[,1][which(varPCA$contrib[,1]>100/nrow(varPCA$contrib)*1.5)]*6, y1=acp$co[,2][which(varPCA$contrib[,1]>100/nrow(varPCA$contrib)*1.5)]*6, length=0.1, col="grey40", lty=5)
  arrows(x0=0, y0=0, x1=acp$co[,1][which(varPCA$contrib[,2]>100/nrow(varPCA$contrib)*1.5)]*6, y1=acp$co[,2][which(varPCA$contrib[,2]>100/nrow(varPCA$contrib)*1.5)]*6, length=0.1, col="grey40", lty=5)

  text(acp$co[,1][which(varPCA$contrib[,1]>100/nrow(varPCA$contrib)*1.5)]*6.8, acp$co[,2][which(varPCA$contrib[,1]>100/nrow(varPCA$contrib)*1.5)]*6.8, 
  labels=substring(names(which(varPCA$contrib[,1]>100/nrow(varPCA$contrib)*1.5)), 2, nchar(names(which(varPCA$contrib[,1]>100/nrow(varPCA$contrib)*1.5)))), col="grey40")
  text(acp$co[,1][which(varPCA$contrib[,2]>100/nrow(varPCA$contrib)*1.5)]*6.2, acp$co[,2][which(varPCA$contrib[,2]>100/nrow(varPCA$contrib)*1.5)]*6.2, labels=substring(names(which(varPCA$contrib[,2]>100/nrow(varPCA$contrib)*1.5)), 2, nchar(names(which(varPCA$contrib[,2]>100/nrow(varPCA$contrib)*1.5)))), col="grey40")
  ...
  text(centroid, labels=c("E2", "E1", "E3", "R", "Wo", "Wi"), font=2)
  legend("topright", legend=expression(bold("a")), bty="n", cex=3.5, inset=c(0, -0.1))
...
#svg(file="Fig5.svg", width=7.5, height=12)
...
### Part a
  ...
  a=ordihull(cbind(acp$li[, 1], -acp$li[, 2]), groups=substring(rownames(acp$li), 8, 10), lty=4, label=F, display=c("sites"))
...
#pdf(file="Fig6.pdf", width=7.5, height=12)
...
a=ordihull(cbind(Htu$X13C, Htu$X15N), groups=paste(substring(Htu$Exp, 1, 2), substring(Htu$Id, 1, 2)), lty=4, label=F, display=c("sites"))
...
text(centroid, labels=c("R", "R", "E1", "E1", "E2", "E2", "E3", "E3", "Wi", "Wi", "Wo", "Wo"), font=2)
...
a=ordihull(cbind(Hfo$X13C, Hfo$X15N), groups=paste(substring(Hfo$Exp, 1, 2), substring(Hfo$Id, 1, 2)), lty=4, label=F, display=c("sites"))
